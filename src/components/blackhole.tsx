  ...
  useEffect(() => {
    ...
    script.onload = () => {
      const sketch = (p: any) => {
        ...
        p.setup = () => {
          const w = width ?? (containerRef.current ? containerRef.current.clientWidth : window.innerWidth)
          const h = height ?? (containerRef.current ? containerRef.current.clientHeight : window.innerHeight)
      ...
      p5Ref.current = new (window as any).p5(sketch)
    }
